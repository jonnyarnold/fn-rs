...
const TYPE_BYTE_NUMBER: u8 = 0x02;

pub const VALUE_BYTES: usize = size_of::<Value>();
...
impl Value {
    ...
    pub fn into_bytes(self) -> [u8; VALUE_BYTES] {
        match self {
            ...
            Value::Number(i) => {
                // [TYPE_BYTE_NUMBER, ...i.to_be_bytes(), 0x00, 0x00, 0x00]
                let mut v = Vec::with_capacity(8);
                v.push(TYPE_BYTE_NUMBER);
    ...
    pub fn from_bytes(bytes: &[u8]) -> Value {
        match bytes[0] {
            ...
            TYPE_BYTE_NUMBER => Value::Number(i32::from_be_bytes(bytes[1..5].try_into().unwrap())),
